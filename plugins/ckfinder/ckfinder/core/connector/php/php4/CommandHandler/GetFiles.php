...
class CKFinder_Connector_CommandHandler_GetFiles extends CKFinder_Connector_CommandHandler_XmlCommandHandlerBase
{
    ...
    function buildXml()
    {
        ...
        $_directThumbAccess = false;

        if (sizeof($files)>0) {
            $_thumbnailsConfig = $_config->getThumbnailsConfig();
            if ($_thumbnailsConfig->getIsEnabled() && $_thumbnailsConfig->getDirectAccess()) {
                $_directThumbAccess = true;
            ...
            foreach ($files as $file) {
                ...
                if ($filemtime !== false) {
                    ...
                    if ($_directThumbAccess && file_exists($_thumbServerPath . $filename)) {
                        $oFileNode[$i]->addAttribute("thumb", "true");
                    }
