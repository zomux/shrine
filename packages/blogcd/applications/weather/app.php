...
function api_location_by_ip($ip){
	...
	$c=0;
	while(!$xml && $c<8){
		$xml = @file_get_contents("http://ipinfodb.com/ip_query.php?ip=".$ip);$c++;
	}
	if(!$xml) return null;
...
class AppBlogcdWeather{
	public $_CONFIG=array(
		'template_engine'=>'comsenz'
	);
	public function main(){
		global $location,$w_current;
		
		$location=RSetting::Load('loc:'.$_SERVER["HTTP_X_FORWARDED_FOR"]);

		if(!$location){
			$location=api_location_by_ip($_SERVER["HTTP_X_FORWARDED_FOR"]);
			if($location) RSetting::Save('loc:'.$_SERVER["HTTP_X_FORWARDED_FOR"],$location);
		}
