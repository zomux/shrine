...
class AppBlogcdPoll{
	...
	private $listPolls=array(
		'1'=>array(
			'title'=> 'Incredible Fast(ping<30ms)',
			'count'=> 1
		),
		'2'=>array(
			'title'=> 'Very Fasy(ping<80ms)',
			'count'=> 4
		),
		'3'=>array(
			'title'=> 'Fast(ping<130ms)',
			'count'=> 2
		),
		'4'=>array(
			'title'=> 'Normal(ping<180ms)',
			'count'=> 0
		),
		'5'=>array(
			'title'=> 'A Bit Slow(ping>180ms)',
			'count'=> 1
		)
	);
	private $titlePoll='wot about de loading speed of ur blog?';
	...
	public function refreshView($list){
		...
		if(in_array($_SERVER['HTTP_X_FORWARDED_FOR'],$arIpPolled)) $need_submit=false;
